...
export class Sessions extends EventEmitter {
  ...
  permission(session, tool, input) {
    ...
    const record = { id: randomUUID(), sessionID: session.id, type: tool, permission: tool, title: `${tool}\n${metadata}`, pattern: [], metadata: input ?? {}, time: { created: Date.now() } };
    return new Promise((resolve) => {
      const timer = setTimeout(() => this.reply(record.id, 'reject'), this.approvalTimeout);
      this.permissions.set(record.id, { record, resolve, timer });
